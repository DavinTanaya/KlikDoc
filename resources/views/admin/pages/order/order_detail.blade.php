...
@push('styles')
  ...
  <style>
    ...
    .status-dropdown:focus {
      outline: none;
      border-color: #4c6ef5;
      box-shadow: 0 0 3px rgba(76, 110, 245, 0.4);
    }
  </style>
@endpush
  ...
  <div class="detail-page">
    ...
    <header class="detail-header">
      <div class="header-container">
        <a href="{{ route('admin.orders.history') }}" class="btn-back">
          ...
          <div class="section-block status-banner">
            <div class="status-info">
              ...
              @if (auth()->user()->role === 'admin')
                <form method="POST" action="{{ route('admin.orders.update', $order->order_code) }}" class="mt-2">
                  @csrf
                  @method('PATCH')

                  <select name="status" class="status-dropdown" onchange="this.form.submit()">
                    <option value="BELUM_BAYAR" {{ $order->status === 'BELUM_BAYAR' ? 'selected' : '' }}>Belum Bayar
                    </option>

    ...
    <main class="history-content">
      <div class="history-container">
        ...
        <div class="status-tabs">
          <a href="{{ route('orders.history') }}" class="tab {{ request()->query('status') ? '' : 'active' }}" style="text-decoration: none">Semua</a>
          <a href="{{ route('orders.history', ['status' => 'BELUM_BAYAR']) }}" class="tab {{ request()->query('status') === 'BELUM_BAYAR' ? 'active' : '' }}" style="text-decoration: none">Belum Bayar</a>
          <a href="{{ route('orders.history', ['status' => 'DIPROSES']) }}" class="tab {{ request()->query('status') === 'DIPROSES' ? 'active' : '' }}" style="text-decoration: none">Diproses</a>
          <a href="{{ route('orders.history', ['status' => 'SELESAI']) }}" class="tab {{ request()->query('status') === 'SELESAI' ? 'active' : '' }}" style="text-decoration: none">Selesai</a>
        </div>
              ...
              <div class="card-footer footer-action">
                ...
                @if ($order->status === 'BELUM_BAYAR')
                  <div class="payment-timer">
                    ...
                    <span>Bayar sebelum <strong>{{ $order->created_at->addDay()->format('d M, H:i') }}</strong></span>
                  </div>

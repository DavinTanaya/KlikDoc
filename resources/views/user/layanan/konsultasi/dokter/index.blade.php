...
@section('body')
        ...
        <div class="sidebar-widget search-widget">
          <form id="filterForm" action="{{ route('konsultasi') }}" method="GET">
            <div class="input-icon-wrapper">
              ...
              <input type="text" name="search" placeholder="Cari dokter, spesialis, RS..." value="{{ $search }}"
                class="search-input"
                onkeydown="if(event.key === 'Enter') document.getElementById('filterForm').submit();">
            </div>

            {{-- Hidden fields to maintain filters --}}
            <input type="hidden" name="kategori_json" id="kategoriInput" value="{{ json_encode($selectedSpecs) }}">
            <input type="hidden" name="filter" id="filterInput" value="{{ request()->query('filter') }}">
          </form>
        </div>

        {{-- ========= JADWAL TERDEKAT (STATIC DULU) ========= --}}
        ...
        <div class="content-header">
          ...
          <div class="sort-wrapper">
            ...
            <select onchange="onSortChange(this.value)">
              ...
              <option value="pengalaman-terlama" {{ request('filter') === 'pengalaman-terlama' ? 'selected' : '' }}>
                Pengalaman Terlama</option>
              <option value="pengalaman-terbaru" {{ request('filter') === 'pengalaman-terbaru' ? 'selected' : '' }}>
                Pengalaman Terbaru</option>
              <option value="nama-a-z" {{ request('filter') === 'nama-a-z' ? 'selected' : '' }}>Nama A-Z</option>
              <option value="nama-z-a" {{ request('filter') === 'nama-z-a' ? 'selected' : '' }}>Nama Z-A</option>
            </select>
          </div>
        </div>
            ...
            <div class="doctor-card">
              ...
              <div class="doctor-info">
                ...
                <div class="price-action">
                  ...
                  <a href="{{ route('konsultasi.detail', $doc->id) }}" class="btn-book-direct">Konsultasi</a>
                </div>
  ...
  ============================== --}}
  <script>
    // Apply filters when checkbox changed
